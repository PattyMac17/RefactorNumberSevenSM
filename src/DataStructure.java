...
public class DataStructure<E extends Obstacle> {
    ...
    public void rightCheck(World w){
        ...
        if(w.player.hitBox.intersects(elem.rightWall)){
            ...
            //w.flag.setVelocity(new Pair(0,0));
            w.allCoins.setVelocity(new Pair(0, 0));
            w.special.setVelocity(new Pair(0,0));
            this.setVelocity(new Pair(0, 0));
            //System.out.println("in right initial check, left side is colliding with right wall");
            //System.out.println("lc: "+ w.player.leftCollision);
        }
        else{
        ...
        while (node.prev != null){
            ...
            if(w.player.hitBox.intersects(elem.rightWall)){
                ...
                w.special.setVelocity(new Pair(0,0));
                this.setVelocity(new Pair(0, 0));
                //System.out.println("in right node.prev check, left side is colliding with right wall");
                //System.out.println("lc: " + w.player.leftCollision);
            }
            else{
                w.player.leftCollision = false;
    ...
    public void leftCheck(World w){
        ...
        if(w.player.hitBox.intersects(elem.leftWall)){
            ...
            w.special.setVelocity(new Pair(0,0));
            this.setVelocity(new Pair(0, 0));
            //System.out.println("in left initial check, right side is colliding with left wall");
            //System.out.println("rc: " + w.player.rightCollision);
        }
        else{
        ...
        while (node.prev != null){
            ...
            if(w.player.hitBox.intersects(elem.leftWall)){
                ...
                //System.out.println("in left node.prev check, right side is colliding with left wall");
                //System.out.println("rc: " + w.player.rightCollision);
            }
            else{
                w.player.rightCollision = false;
    ...
    //player-NPC interaction ONLY USE FOR allNPCs
